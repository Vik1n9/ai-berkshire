...
- 或：`我的持倉`（如果已有儲存的組合檔案 `reports/組合管理/portfolio-latest.md`）
...
同時檢查是否存在已有的組合檔案（`reports/組合管理/portfolio-latest.md`），如有則讀取並更新。
...
將組合資訊寫入 `reports/組合管理/portfolio-latest.md`（持倉組合橫跨多家公司，不屬於單一個股，放 reports/組合管理/ 目錄），包含：
- 最新持倉表
- 本次審視日期和結論
- 調倉記錄（追加）
- 下次審視提醒

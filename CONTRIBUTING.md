...
感謝你對 AI Berkshire 的關注。這個專案起點是我個人的投資研究流程，開源出來是希望方法論對更多人有用。為了讓維護可持續，請在提交 issue 或 PR 前花兩分鐘讀完這份指南。

## 歡迎的貢獻

- **Skill 的 bug 修復**：提示詞錯漏、流程跑不通、輸出格式問題等。請在 PR 描述裡說明：哪個 skill、什麼場景下出的問題、修復後你實際跑過的結果。
- **Skill 的改進**：讓分析更嚴謹、資料交叉驗證更可靠、輸出更可用的改動。改動需要附帶說明"為什麼這樣更好"，最好有改動前後的輸出對比。
- **檔案改進**：README、docs/ 中的錯別字、失效連結、表述不清的修正；英文版與中文版內容不同步的修正。
- **相容性適配**：Claude Code / Codex 新版本帶來的適配問題。
- **用本框架產出的研究報告**：歡迎提交到 `REPORT/community/` 子目錄（按 `REPORT/community/[公司名]/` 組織，僅限美股與台股公開發行公司）。要求：① 報告確實是用本倉庫的 skill 跑出來的，並在文首註明所用 skill 和模型；② 附上免責宣告（不構成投資建議）；③ 一個 PR 一份報告。質量明顯不達標（如未用框架、純手寫觀點、缺關鍵財務交叉驗證）的會被關閉。
- **新 Skill 提議**：請先開 issue 討論，說明這個 skill 解決什麼研究場景、和現有 18 個 skill 的邊界是什麼。直接提交大型新功能 PR 而沒有事先討論的，可能會被關閉——不是否定你的工作，而是方向沒對齊時雙方都浪費時間。

## 不接受的貢獻

- **修改 `REPORT/` 下已有的報告（`community/` 除外）、`實盤記錄/`、`篩選公司/`**：這些是我個人的研究產出和交易記錄，不接受外部修改。你自己的研究請提交到 `REPORT/community/`（見上），或在自己的倉庫釋出後在 issue 裡分享連結。
- **投資觀點之爭**：報告裡的結論（比如某公司值不值得買）是方法論執行的結果，不接受以"我不同意這個結論"為由的修改。對方法論本身的質疑歡迎開 issue 討論。
- **純樣板檔案類 PR**：由自動化工具批次生成、與專案實際需求無關的配置檔案或治理檔案。
- **大規模格式化/重新命名**：不改變實質內容、只產生大量 diff 噪音的改動。

## 提交 PR 的基本要求

1. 一個 PR 只做一件事，保持 diff 聚焦。
2. 描述裡寫清楚：改了什麼、為什麼改、怎麼驗證的。涉及 skill 的改動，請附上你在 Claude Code 或 Codex 裡實際執行的效果。
3. 中文或英文均可。
4. 改動 README.md 中影響內容的部分時，請同步更新 README_EN.md（反之亦然）；純排版修正可以只改一邊。

## 提交 Issue

請從三個 Issue 模板中選擇對應型別提交（已關閉空白 issue）。**核心要求：任何觀點都要附具體、可復現的例子**，否則無法核實，會被要求補充或直接關閉。

- **🐞 Skill 報錯**：註明呼叫的 skill（確保標題與正文一致）、完整命令、報錯資訊、**使用的模型和客戶端版本**。
- **📉 資料錯誤**（股價/市值/財務資料）：給出具體標的、錯誤數值、**你認為的正確值和來源連結**，並說明是否涉及復權/送轉/增發/匯率等口徑。
- **💡 改進建議**：給出**具體出處**（哪份報告、哪張表）和**期望的改進對比**（當前長這樣 → 期望長這樣），不接受只有抽象描述的建議。
- **研究請求**（"幫我分析某某公司"）：本專案不接單。這套框架的意義就是讓你自己跑出專業級研究——照著 README 的快速開始，用你自己的 AI 工具執行即可。

## 安全問題

發現 API 金鑰洩露、提示注入等安全漏洞，請**不要**開公開 issue，按 [SECURITY.md](SECURITY.md) 的方式私下報告。

## 免責宣告

本專案的一切內容僅為研究方法論的展示，不構成投資建議。貢獻者提交的內容同樣預設遵循這一原則，請勿在貢獻中加入薦股、收益承諾類內容。
...
- **Research reports produced with this framework** — submit to the `REPORT/community/` subdirectory (organized as `REPORT/community/[company]/`, US and Taiwan publicly listed companies only). Requirements: (1) the report was actually generated with this repo's skills — state the skill and model used at the top; (2) include a disclaimer (not investment advice); (3) one report per PR. Reports that clearly miss the bar (framework not used, hand-written opinions only, missing financial cross-validation) will be closed.
...
- **Edits to existing reports under `REPORT/` (except `community/`), `實盤記錄/` (live trading records), or `篩選公司/`** — these are my personal research output and trading records. Submit your own research to `REPORT/community/` (see above), or publish it in your own repo and share the link in an issue.

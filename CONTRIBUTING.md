...
- **文件改進**：README、docs/ 中的錯別字、失效連結、表述不清的修正；英文版與中文版內容不同步的修正。
- **相容性適配**：Claude Code / Codex 新版本帶來的適配問題。
- **用本框架產出的研究報告**：歡迎提交到 `reports/community/` 子目錄（按 `reports/community/[公司名]/` 組織）。要求：① 報告確實是用本倉庫的 skill 跑出來的，並在文首註明所用 skill 和模型；② 附上免責宣告（不構成投資建議）；③ 一個 PR 一份報告。質量明顯不達標（如未用框架、純手寫觀點、缺關鍵財務交叉驗證）的會被關閉。

...
- **用本框架產出的研究報告**：歡迎提交到 `reports/community/` 子目錄（按 `reports/community/[公司名]/` 組織，僅限美股與台股公開發行公司）。要求：① 報告確實是用本倉庫的 skill 跑出來的，並在文首註明所用 skill 和模型；② 附上免責宣告（不構成投資建議）；③ 一個 PR 一份報告。質量明顯不達標（如未用框架、純手寫觀點、缺關鍵財務交叉驗證）的會被關閉。
- **新 Skill 提議**：請先開 issue 討論，說明這個 skill 解決什麼研究場景、和現有 18 個 skill 的邊界是什麼。直接提交大型新功能 PR 而沒有事先討論的，可能會被關閉——不是否定你的工作，而是方向沒對齊時雙方都浪費時間。

## 不接受的貢獻

- **修改 `reports/` 下已有的報告（`community/` 除外）、`實盤記錄/`、`篩選公司/`**：這些是我個人的研究產出和交易記錄，不接受外部修改。你自己的研究請提交到 `reports/community/`（見上），或在自己的倉庫釋出後在 issue 裡分享連結。
...
- **Research reports produced with this framework** — submit to the `reports/community/` subdirectory (organized as `reports/community/[company]/`, US and Taiwan publicly listed companies only). Requirements: (1) the report was actually generated with this repo's skills — state the skill and model used at the top; (2) include a disclaimer (not investment advice); (3) one report per PR. Reports that clearly miss the bar (framework not used, hand-written opinions only, missing financial cross-validation) will be closed.
